...
**const 指令**
const 指令声明一个**只读**的常量。无法更改
```
    const num = 1
    num += 1
```
如果使用 const 声明变量，你必须在声明的同时初始化这个变量，因为他是不可变的
你不能使用像 let、var 这样的方式，如果你只声明，不赋值，就会报错。
```
    let num1;
    var num2;
    const num3; // 这样不合法， const 一旦声明必须要立即初始化。
```
const 的作用域与 let 相同，只在当前块级作用域下及生效。声明前无效。

**const 原理**
const 保证的不是值的不能变化，而是变量指向的内存地址的不能变化，对简单数据类型来讲（数字，字符串，布尔），值就保存在变量指向的内存地址中，因此等同于常量。对于引用类型来讲，变量保存的内存地址，是一个指针。
所以针对引用类型，const 只能保证他的地址指针不变，但是内部的数据是无法保证的。因此如果将一个对象声明为常量要注意。
